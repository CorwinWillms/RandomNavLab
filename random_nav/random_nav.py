...
=======
    ...
    def send_goal(self):
        ...
        if self.map is not None:
            self.goal.pose.pose.position.x = np.random.rand() * (self.map.width * self.map.resolution) + self.map.origin_x
            self.goal.pose.pose.position.y = np.random.rand() * (self.map.height * self.map.resolution) + self.map.origin_y
            ...
            if val == 100:
                ...
                state = "unknown"
            self.get_logger().info("HEY! Map position ({:.2f}, {:.2f}) is {}".format(x, y, state))
            while state == "unknown":
                self.get_logger().info("HEY! WRONG POSITION PICKING ANOTHER ONE")
                self.goal.pose.pose.position.x = np.random.rand() * (self.map.width * self.map.resolution) + self.map.origin_x
                self.goal.pose.pose.position.y = np.random.rand() * (self.map.height * self.map.resolution) + self.map.origin_y
                x = self.goal.pose.pose.position.x
                y = self.goal.pose.pose.position.y
            ...
            self.goal.pose.pose.position.x = 0.0
            self.goal.pose.pose.position.y = 0.0
>>>>>>> 2ccc94dbc0869c3df13007b54825ba2319d68da9

        quaternion = transformations.quaternion_from_euler(0, 0, np.random.rand() * 360, 'rxyz')
        self.goal.pose.pose.orientation.x = quaternion[0]
        self.goal.pose.pose.orientation.y = quaternion[1]
        self.goal.pose.pose.orientation.z = quaternion[2]
        self.goal.pose.pose.orientation.w = quaternion[3]
...
def main():
    ...
    node = NavNode(0.0, 0.0, 0.0, float('inf'))
